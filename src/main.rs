...
struct StatusResponse {
    timestamp: String,
    forbidden_processes: Vec<String>,
    platform: String,
}
...
struct StatusQuery {
    #[serde(default)]
    include_topmost: bool,
}

// Default forbidden processes for each platform
fn get_default_forbidden_list() -> Vec<String> {
    let mut forbidden = Vec::new();
    
    // Cross-platform applications
    forbidden.extend([
        "Code.exe".to_string(), "code".to_string(),                    // VS Code
        "devenv.exe".to_string(), "devenv".to_string(),                // Visual Studio
        "idea64.exe".to_string(), "idea".to_string(), "IntelliJ IDEA".to_string(), // IntelliJ IDEA
        "PyCharm".to_string(), "pycharm".to_string(),                  // PyCharm
        "eclipse".to_string(), "Eclipse".to_string(),                  // Eclipse
        "atom".to_string(), "Atom".to_string(),                        // Atom
        "sublime_text".to_string(), "Sublime Text".to_string(),        // Sublime Text
        "notepad++.exe".to_string(), "Notepad++".to_string(),          // Notepad++
        "vim".to_string(), "nvim".to_string(), "emacs".to_string(),                // Terminal editors
        "AutoHotkey.exe".to_string(), "autohotkey".to_string(),        // AutoHotkey
        "obs64.exe".to_string(), "obs".to_string(), "OBS Studio".to_string(),      // OBS Studio
        "PowerToys.exe".to_string(), "PowerToys".to_string(),          // PowerToys
        "ollama".to_string(), "Ollama".to_string(),                    // Ollama
        "github desktop".to_string(), "GitHub Desktop".to_string(),    // GitHub Desktop
        "sourcetree".to_string(), "SourceTree".to_string(),            // SourceTree
        "postman".to_string(), "Postman".to_string(),                  // Postman
        "insomnia".to_string(), "Insomnia".to_string(),                // Insomnia
        "docker".to_string(), "Docker Desktop".to_string(),           // Docker
        "virtualbox".to_string(), "VirtualBox".to_string(),           // VirtualBox
        "vmware".to_string(), "VMware".to_string(),                   // VMware
        "wireshark".to_string(), "Wireshark".to_string(),             // Wireshark
        "fiddler".to_string(), "Fiddler".to_string(),                 // Fiddler
        "burp".to_string(), "Burp Suite".to_string(),                 // Burp Suite
        "ida".to_string(), "IDA Pro".to_string(),                     // IDA Pro
        "ghidra".to_string(), "Ghidra".to_string(),                   // Ghidra
        "x64dbg".to_string(), "x32dbg".to_string(),                   // x64dbg/x32dbg
        "ollydbg".to_string(), "OllyDbg".to_string(),                 // OllyDbg
        "cheat engine".to_string(), "Cheat Engine".to_string(),       // Cheat Engine
        "process hacker".to_string(), "Process Hacker".to_string(),   // Process Hacker
        "process monitor".to_string(), "Process Monitor".to_string(), // Process Monitor
        "autoruns".to_string(), "Autoruns".to_string(),               // Autoruns
        "regshot".to_string(), "Regshot".to_string(),                 // Regshot
    ]);
...
fn enumerate_topmost_processes() -> Vec<String> {
    ...
    // This could be implemented using platform-specific APIs if needed
    Vec::new()
}

fn detect_forbidden_processes(forbidden_list: &[String], include_topmost: bool) -> Vec<String> {
    let mut sys = System::new_all();
    sys.refresh_processes();
